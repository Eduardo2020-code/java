...
/**
 *Esta clase nos muestra la interfaz para cambiar el telefono de un empleado
...
public class TelefonoGUI extends javax.swing.JFrame {
    ...
    /**
     * Este metodo hace el llamado a los metodos para acceder a la base de datos y actualizar el telefono
     * @throws SQLException 
     */
    public void actualizarTelefono() throws SQLException{
        ...
        ArrayList<DatosEmpleados> lista = c.datosModificacionTelefono();
        ...
        for(int i = 0; i<lista.size(); i++){
            ...
            if(lista2[i][0].equals(tfUsuario.getText())){
                user=lista2[i][0];
                contra=lista2[i][1];
                telefonoAnt=lista2[i][2];
            }
        }
        if(tfUsuario.getText().length()!=0
                && tfContrasenia.getText().length()!=0
                && tfTelefonoNuevo.getText().length()!=0){
            if(user.equals(tfUsuario.getText())){
                if(contra.equals(tfContrasenia.getText())){
                    if(!telefonoAnt.equals(tfTelefonoNuevo.getText())){
                        try {
                            DatosEmpleados telefonoActualizar = new DatosEmpleados();

                            telefonoActualizar.setUsuario(tfUsuario.getText());
                            telefonoActualizar.setContrasenia(tfContrasenia.getText());
                            telefonoActualizar.setTelefonoNuevo(tfTelefonoNuevo.getText());

                            DatosEmpleados telefonoActualizado =null;
                            DatosEmpleadosDao d = new DatosEmpleadosDao();

                            telefonoActualizado = d.actualizarTelefono(telefonoActualizar);

                            if(telefonoActualizado != null){
                                JOptionPane.showMessageDialog(null, "El telefono del empleado se actualizó correctamente");
                            ...
                            Logger.getLogger(TelefonoGUI.class.getName()).log(Level.SEVERE, null, ex);
                        }
                    }else{
                        JOptionPane.showMessageDialog(null, "El nuevo telefono no puede ser igual al anterior");
                    }
                }else{
    ...
    /**
     * este metodo consulta la base de datos para corroborar que los datos no se repitan al hacer la actualizacion
     * @param consulta
     * @return arreglo de dos dimensiones
     */
    public String[][] formatoRegistros(ArrayList<DatosEmpleados> consulta){
        ...
        for (int i = 0; i < consulta.size(); i++) {            
            ...
            registros[i][2] = consulta.get(i).getTelefono();
        }
    ...
    private void initComponents() {
        ...
        tfTelefonoNuevo = new javax.swing.JTextField();
        ...
        jLabel21.setText("Nuevo telefono:");
        jPanel2.add(jLabel21, new org.netbeans.lib.awtextra.AbsoluteConstraints(250, 270, -1, -1));

        tfTelefonoNuevo.setFont(new java.awt.Font("Decker", 0, 14)); // NOI18N
        tfTelefonoNuevo.setForeground(new java.awt.Color(153, 153, 153));
        tfTelefonoNuevo.setBorder(javax.swing.BorderFactory.createBevelBorder(javax.swing.border.BevelBorder.RAISED));
        jPanel2.add(tfTelefonoNuevo, new org.netbeans.lib.awtextra.AbsoluteConstraints(400, 270, 170, 25));
        ...
        titulo.setText("CAMBIANDO TELEFONO DE EMPLEADO");
        jPanel2.add(titulo, new org.netbeans.lib.awtextra.AbsoluteConstraints(170, 50, -1, -1));
    ...
    private javax.swing.JTextField tfTelefonoNuevo;
